...
const ClimbingMap = ({ climbingPrefectures, mapData, tabState }) => {
  ...
  const initializeMap = useCallback(
    ...
    [climbingPrefectures, mapData, onEachFeature],
  );
  ...
  const resetMap = useCallback(
    ...
    [initializeMap, setFilterPrefectures],
  );
  ...
  useEffect(() => {
    if (!mapRef.current && tabState) {
      ...
      legendRef.current.onAdd = function () {
        ...
        labels.push(`<p class="${styles.legend__title}">登頂座数</p>`);
  ...
  useEffect(() => {
    ...
    // クリーンアップ処理
    return () => {};
  }, [resetMap, searchParams, setFilterPrefectures]);
