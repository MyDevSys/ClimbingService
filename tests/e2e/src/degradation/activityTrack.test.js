...
test("活動トラック：地図への表示内容とボタン操作", async () => {
  ...
  expect(popupTitle).toHaveText("竈門神社");
  ...
  for (let index = 0; index < popupTimeText.length; index++) {
    ...
    expect(popupTime).toHaveText(popupTimeText[index]);
  }
  ...
  const pathElements = newPage.locator('path[class*="leaflet-interactive"]');
  ...
  for (let i = 0; i < targetButton.length; i++) {
    ...
    // 休憩ポイントボタンをONにした際に表示される1番目のアイコンのポップアップの確認試験
      ...
      const restPopupTitle = await restPopupElement
        ...
        .nth(0);
      const titleContent = await restPopupTitle.textContent();
      expect(titleContent).toEqual("休憩時間 : 6分");
